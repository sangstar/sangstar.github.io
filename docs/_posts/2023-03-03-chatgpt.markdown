...
In order to remedy this, the probability of a token being chosen by the model is modified by the *freezing function*:

$$\widetilde p_i = f_\tau (p)_i = \frac{p_i^{1/\tau}}{\sum_j p_j^{1/\tau}}$$
