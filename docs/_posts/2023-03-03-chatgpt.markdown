...
To describe as much as possible, I talked about an encoder-decoder transformer, but not all LLM's are based off this design. Some, like BART, are in this framework, while BERT is an encoder-only model, and LLM's like GPT (all generations), Bard and LlaMa are decoder-only. 
