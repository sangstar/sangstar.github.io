...
which is fed to a feed-forward neural network as essentially an "enhanced" version of a word embedding, holding more contextual information through self attention, usually then passed to an $$\text{Add&Norm}$$ layer which adds the input used to generate the output with that latter output. That sounds confusing, but for example the first $$\text{Add&Norm}$$ layer adds the input embeddings with the result of the first multi-head attention layer, and the second layer adds the output of the feed-forward layer to the output of the multiheaded attention layer (which serves as the $$\text{Add&Norm}$$ input as it used as output for the feed-forward). $$\text{Add&Norm}$$ layers help to reduce the risk of vanishing gradients. 
