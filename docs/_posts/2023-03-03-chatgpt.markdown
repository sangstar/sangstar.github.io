...
which for me, when I first saw it, was absolutely befuddling. My hope is that after reading this article, if you go back to this graphic it'll make a bit more sense. Let's dive right and start dealing with the attention units. I will be describing a an autoregressive encoder-decoder transformer to generate text.
...
To describe as much as possible, I talked about an encoder-decoder transformer, but not all LLM's are based off this design. Some are, like BART, are in this framework, while BERT is an encoder-only model, and LLM's like GPT (all generations), Bard and LlaMa are decoder-only. 

Decoder-only transformers are more popular for autoregressive text generation. An autoregressive transformer simply does not have much use for an encoder's output. The encoder, which has bidirectional self-attention (no masking) is not necessary for autoregression as multi-directional context is not applicable to generating sequences token-by-token and ends up providing needless bloat to an already incredibly computationally-hefty model architecture. Not only is it wasteful; it can be detrimental. For generating text, only the currently generated tokens should be attended to -- not the entire input sequence it's learning off of. 

It is theoretically possible (but not popular) to use a encoder-decoder framework for autoregressive textgen, and I explained it in this context just to explain what both encoders and decoders were. 
