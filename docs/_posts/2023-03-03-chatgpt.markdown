...
$$\text{Attention}(q_i, k_j) = \alpha_{i,j} = \frac{q_i^T k_j}{\sqrt{n}}$$

which can be thought of as the relevance of token $$i$$ on token $$j$$. These attention weight are computed for all $$n$$ tokens, so that for token $$i$$ you have a vector of attention units $$(\alpha_{i,1}, \alpha_{i,2}, ... , \alpha_{i,n})$$ which includes $$\alpha_{i,i}$$, for completeness sake. This vector then acts as weight for the weighted sum of value vectors for each position:

$$a_i = \sum_{j=1}^n \alpha_{i,j} v_j$$

where $$a_i$$ is known as the *attention* for token or position $$i$$. 
