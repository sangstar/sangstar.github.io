...
That's how a generative transformer like GPT works. As you can see, this process isn't stochastic, yet if you regenerate text in ChatGPT it'll usually produce different responses. This is thanks to something called *semantic temperature*, or just *temperature*. 

# Semantic temperature
If you ask ChatGPT a question, and click "Regenerate response", it'll almost always give you a unique response. Yet none of what I told you involves any random processes. 

The reason why this happens is because at the output layer a probability distribution of the most likely next token is formed out of all tokens in the model's vocabulary. If the most probable token is chosen each time, the output is notoriously banal, repetitive, and overall uninteresting. 

In order to remedy this, the probability of a token being chosen by the model is modified by the *freezing function*. Recall that the final layer of a transformer is a softmax scaled on output $$z$$:

$$\sigma(z_i) = \frac{e^{z_i}}{\sum_j^N e^{z_i}}$$

but introduce an additional parameter $$\tau$$ like so:

$$\sigma(z_i, \tau) = \frac{e^{z_i/\tau}}{\sum_{j=0}^{N=1} e^{z_i/\tau}}$$

where $$\tau$$ is the semantic temperature. In order to better understand how this affects the output logits, consider some edge cases. 

1. $$\tau \to 1$$. In this case, the denominator is unity and the numerator is $$p_i$$, which means $$f_{\tau =1} (p)_i = p_i$$ which means $$f_{\tau =1}$$ is the original softmax. 

2. $$\tau \to 0$$ this reduces to *argmax*. See [EDZ's answer on stack exchange](https://math.stackexchange.com/questions/2656231/proving-that-softmax-converges-to-argmax-as-we-scale-x) where $$1/\tau \to 0$$ is equivalent to $$c \to \infty$$ in the user's example.


Generally speaking, when temperature is computed it's usually applied to the logits $$\text{logit} = \frac{\text{logit}}{\tau}$$ before being applied to the softmax. At lower temperature $$\tau$$, the resulting probability distribution over the classes becomes exponentially "sharper", exacerbating the gap between higher probability and lower probability classes, hence why $$\tau -> 0$$ becomes argmax. Larger values of $$\tau$$ will blunt the probability distribution over the classes, allowing previously lesser probability tokens to have a higher chance of being generated. The softmax in general is an analog to the Gibbs distribution in physics. In the Boltzman distribution, lower temperature causes the Boltzmann distribution to be more pointed. 

Below is a plot of the Maxwell-Boltzman distribution as temperature varies. It's not the exact same as the Gibbs distribution or temperature-dependent softmax, but it captures the same blunting with increased temperature. Note that here $$a \propto T$$.
...
A general outline of the fine-tuning process can be visualized with a gorgeous visual from HuggingFace's [blogpost](https://huggingface.co/blog/rlhf) on RLHF, which explains this whole process in great detail:
...
It may seem like no explanation as to how ChatGPT was trained seems to make its remarkable usefulness believable. My only explanation for this would be the exorbitant amount of training data and parameters at play here.

ChatGPT released their API recently, and with it, an option to control its output temperature. It would be interesting to see how it does with more "safe" or exploratory responses. 
...
There are likely *tons* of things under the hood that ChatGPT benefits from that OpenAI has not publically disclosed. I'm not aware of how it handles continuing conversations or correcting itself beyond a few complete guesses.

One of the limitations listed on ChatGPT's main page in the chat window is that it has "Limited knowledge of world and events after 2021". That implies that there is sparse training data that exists in 2021 or later. One of the biggest issues with serving trained models is *concept drift*, which is a model's declining usefulness with time since training. I suspect ChatGPT's advice can and will become dated in the future, and it will probably be up for a costly re-training, and, given how widely used it is, in the near future, which will prevent a constant financial load to provide ChatGPT as a service on the part of OpenAI. Given its funding recently, however, I'm not too worried for them.  
