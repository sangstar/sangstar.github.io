...
Generally speaking, when temperature is computed it's usually applied to the logits $$\text{logit} = \frac{\text{logit}}{\tau}$$ before being applied to the softmax. At lower temperature $$\tau$$, the resulting probability distribution over the classes becomes exponentially "sharper", exacerbating the gap between higher probability and lower probability classes, hence why $$\tau -> 0$$ becomes argmax. Larger values of $$\tau$$ will blunt the probability distribution over the classes, allowing previously lesser probability tokens to have a higher chance of being generated. The softmax in general is an analog to the Gibbs distribution in physics. In the Gibbs distribution, lower temperature cause the distribution to be more pointed. 
