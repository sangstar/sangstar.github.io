...
$$\sigma(z_i, \tau) = \frac{e^{z_i/\tau}}{\sum_j^N e^{z_i/\tau}}$$
...
1. $$\tau \to 1$$. In this case, the denominator is unity and the numerator is $$p_i$$, which means $$f_{\tau =1} (p)_i = p_i$$ which means $$f_{\tau =1}$$ is an identity function. 

2. $$\tau \to 0$$ this reduces to an *argmax*. See [EDZ's answer on stack exchange](https://math.stackexchange.com/questions/2656231/proving-that-softmax-converges-to-argmax-as-we-scale-x) where $$1/\tau \to 0$$ is equivalent to $$c \to \infty$$ in the user's example.
