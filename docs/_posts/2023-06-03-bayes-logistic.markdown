...
$$P(w_1, w_2, ..., w_n \vert c) = P(w_1 \vert c) \cdot P(w_2 \vert c) \ \cdot ... \cdot P(w_{n-1} \cdot P(w_n \vert c)$$

Thus our naive Bayes classifier is:

$$\hat c = c_{NB} = \underset{c \in C}{\text{argmax}} \ P(c) \ \underset{i}{\prod} P(w \vert c)$$
