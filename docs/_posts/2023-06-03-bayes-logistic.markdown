...
The naive Bayes model for classification in NLP is the next "canonical" jump from an $$n$$-gram model. The goal of all classifiers $$\hat c$$ is to satisfy the following:

$$c = \text{argmax}_{c \in C} \ P(c|d)$$