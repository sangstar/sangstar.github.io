...
Large language models are models trained on typically billions of parameters. Since 2018 they've pretty always used the transformer architecture. In NLP, transformer LLM's are a nuclear power. They have no other competition besides themselves. I struggle to think of a use case so niche that a custom-trained model would outperform an LLM applied downstream. 

LLM's are fine-tuned for downstream tasks, although they can be applied downstream without any modifications and often do perfectly well (zero-shot learning). Traditionally, this was through traditional fine-tuning, a form of transfer learning in which you train a model with its pre-trained weights rather than typical weight initialization techniques. This has a risk of suffering from *catastrophic inference*, or catastrophic forgetting.
